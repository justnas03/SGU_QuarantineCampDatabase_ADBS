...
<head>
    ...
    <title>CV19 Camp - Xét nghiệm</title>
</head>
...
<body>
    ...
    xn.SPO2, xn.Respiratiory_Rate, xn.QT_result, xn.ct, xn.Warning FROM xetnghiem xn ORDER BY 'Ma_DT' ASC LIMIT " . $itemOfPage . " OFFSET " . $offset . "";
    ...
    <div class="home_content">
        ...
        <div class="add-new-obj">
            <form action="./add_obj/themxetnghiem.php" method="post">
                <input type="submit" value="Thêm xét nghiệm">
            </form>
        </div>
        <div class="table-BN">
            <table>
                <tr class="tr-title">
                    <th>Mã xét nghiệm</th>
                    <th>Mã bệnh nhân</th>
                    <th>Thời gian bắt đầu xét nghiệm</th>
                    <th>PCR Test</th>
                    <th>SPO2</th> 
                    <th>Respiratiory Rate</th>
                    <th>Quick Test</th>
                    <th>CT</th>
                    <th>Warning Mark</th>
                </tr>
...
if (isset($_POST['search'])){
    ...
    $sqlSearch = "SELECT * FROM xetnghiem WHERE MaXN LIKE '%$searchTerm%'";
    $resultSearch = $db->execute($sqlSearch);

    if($resultSearch){
        while($row = $db->getData()){   
?>
             <tr class="data-table" data-href="./detail/detailPatient.php?id=<?= $row['MaBN'] ?>" ></tr>
                                <td><?php echo $row["MaXN"]; ?></td>
                                <td><?php echo $row["MaBN"]; ?></td>
                                <td><?php echo $row["ThoigianXN"]; ?></td>
                                <td><?php echo $row["PCR_result"]; ?></td>
